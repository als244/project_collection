...
abstract class AbstractCore implements CoreInterface
{
    ...
    public function mySearch(string $baseURL, string $queryString){
        ...
        // $url = 'https://utas.external.s3.fut.ea.com/ut/game/fifa20/transfermarket?maskedDefId=153079&type=player&_' . $time;
...
//                'redirect_uri'  => 'nucleus:rest',
        ...
        if ((string) $url !== 'https://www.easports.com/fifa/ultimate-team/web-app/auth.html') {
            ...
            if (strpos($responseContent, $locale->get('login.security_code')) !== false) {
                if ($code == null){
                    $code = readline("Enter Security Code: ");
                }
                if ($code === null) {
    ...
    public function bid($tradeId, $price) : BidResponse
    {
        try {
            ...
            //print("permission denied buying\n");
        }

        if (is_null($response)){
            return new BidResponse([], -1);
        }
    ...
    public function sell($itemId, $bid, $bin, int $duration = 3600) : ?TradeItemInterface
    {
        ...
        try{
             ...
             print("permission denied listing\n");
             return null;
        }
    ...
    public function watchlistDelete($tradeId)
    {
        ...
        try{
            ...
            return new WatchlistResponse(-1, -1, []);
        }
